...
export function DashboardPage() {
  ...
  const { leads, isLoading, error, totalPages, totalLeads, currentPage, refetch } = useLeads({
    ...filters,
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {totalLeads > 0 ? `${totalLeads} total leads` : 'Manage your pipeline'}
          </p>
        </div>
        <Button
      ...
      <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState
            action={{ label: 'Create first lead', onClick: () => navigate('/leads/create') }}
          />
        ) : (
          <>
            {/* Desktop table */}
            <div className="hidden md:block">
              <LeadTable leads={leads} isAdmin={isAdmin} onDeleteClick={setDeletingLead} />
            </div>
